...
module.exports = ctx => {
  ...
  const NAV = [
    ...
    ['quotes', 'Запросы КП', 'i-ui-quote', 'kabinet/zaprosy-kp/', D.quotes.length],
...
<div class="pk-cab-body">
  <nav aria-label="Хлебные крошки" class="pk-cab-crumbs"><a href="${BASE}">Главная</a><span aria-hidden="true">/</span><a href="${BASE}kabinet/">Кабинет</a>${crumb ? `<span aria-hidden="true">/</span><a href="${BASE + crumb[1]}">${esc(crumb[0])}</a>` : ''}<span aria-hidden="true">/</span><span aria-current="page">${esc(h1)}</span></nav>
...
<script type="application/json" id="pk-cab-data">${seed}</script>
  ...
  const pages = [
    ...
    page('kabinet/dokumenty/', 'Счета и УПД', 'Счета, УПД, акты сверки и договоры: фильтры по типу и периоду, статусы ЭДО.', 'Счета и УПД',
      { key: 'documents', sub: 'Закрывающие документы по заказам. Подписание — через оператора ЭДО.' }),
  ...
  for (const o of D.orders) {
    if (o.page) continue; // ПК-10428 — экран макета
